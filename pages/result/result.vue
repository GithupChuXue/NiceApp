	...
	<view class="result">
		...
		<view class="results">
			<uni-list>
				...
				<uni-list-item direction="column" v-for="item in results" :key="item.id">
					<!-- 通过header插槽定义列表的标题 -->
...
<script>
	...
	export default {
		...
		data() {
			return {
				results: read.read_results("../static/data_test/data_local.json"),
				status: "more",
				contentText: {
					contentdown: "更多精彩",
					contentrefesh: "加载中",
		...
		onPullDownRefresh() {
			console.log("上拉刷新");
			this.results = read.read_local("../static/data_test/data_local.json");
			console.log("上拉刷新已完成");
		},
